...
import { beautyTestimonialsFull } from "@/lib/data";
...
export function TestimonialCarouselBeauty() {
  const list = beautyTestimonialsFull;
  const n = list.length;
      ...
      <div className="max-w-5xl mx-auto">
        <ScrollReveal className="text-center mb-14">
          ...
          <p className="text-slate-400 text-base lg:text-lg max-w-2xl mx-auto leading-relaxed break-keep">
            뷰티·의료 업종에서 BELLO와 함께한 병원·클리닉이 남긴 후기입니다.
            <br className="hidden md:block" />
            의료법을 지키면서 노출과 예약이 늘어난 사례를 소개합니다.
          </p>
        </ScrollReveal>
        ...
        <ScrollReveal>
            ...
            {[-1, 0, 1].map((offset) => {
              ...
              return (
                <motion.div
                  ...
                  style={isCenter ? { scale: 1, opacity: 1, zIndex: 10, x: dragX } : { scale: 0.9, opacity: 0.5, zIndex: 0 }}
                ...
                >
                  ...
                  <blockquote className={`text-slate-200 leading-relaxed flex-1 break-keep ${isCenter ? "text-sm md:text-base line-clamp-6" : "text-xs line-clamp-4"}`}>
                    {t.quote}
                  </blockquote>
          ...
          <div className="flex items-center justify-center gap-4 mt-10">
            <button type="button" onClick={() => go(-1)} className="p-2.5 rounded-xl bg-white/10 text-slate-300 hover:text-white transition-colors border border-white/10" aria-label="이전">
              <ChevronLeft className="w-5 h-5" />
            </button>
            <span className="text-sm text-slate-400 min-w-[3rem] text-center break-keep">{current + 1} / {n}</span>
            <button type="button" onClick={() => go(1)} className="p-2.5 rounded-xl bg-white/10 text-slate-300 hover:text-white transition-colors border border-white/10" aria-label="다음">
