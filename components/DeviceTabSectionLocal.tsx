...
const TABS = [
  { id: "naver", label: "네이버 플레이스", icon: MapPin },
  { id: "daangn", label: "당근마켓", icon: MessageCircle },
  { id: "catch", label: "캐치테이블", icon: Utensils },
];

const CONTENTS: Record<string, { title: string; desc: string }> = {
  naver: { title: "네이버 플레이스", desc: "영수증·예약 기반 리뷰로 검색 1페이지 안착. 블라인드 리스크 제로화." },
  daangn: { title: "당근마켓·동네생활", desc: "단골 랭킹·경험담 바이럴로 지역 신뢰도와 오프라인 유입을 동시에." },
  catch: { title: "캐치테이블 웨이팅", desc: "웨이팅 수 증가로 입소문·재방문률 상승. 줄 서는 핫플 만들기." },
};

export function DeviceTabSectionLocal() {
      ...
      <div className="max-w-5xl mx-auto">
        <ScrollReveal className="text-center mb-14">
          ...
          <p className="text-slate-400 text-base lg:text-lg max-w-2xl mx-auto leading-relaxed break-keep">
            네이버 플레이스, 당근, 캐치테이블을 한 팀에서 설계·실행합니다.
          </p>
        </ScrollReveal>
        <ScrollReveal>
